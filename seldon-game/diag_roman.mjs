...
try {
  ...
  console.log('Build output:', e.stdout?.toString().slice(-2000));
  process.exit(1);
}
...
function runTest(seed, size, shape, maxPhases) {
  ...
  let dominance40Phases = 0;
  let dominance30Phases = 0;
  let longestRun40 = 0;
  let longestRun30 = 0;
  let currentRun40 = 0;
  let currentRun30 = 0;
  ...
  return { seed, size, shape, peakPct: (peakPct*100).toFixed(1), peakPhase, longestRun40, longestRun30, snapshots };
}
...
for (const t of tests) {
  ...
  for (const s of r.snapshots) {
    console.log(`  Ph${s.phase}: ${s.empires} empires | Top3: ${s.top3} | Leader=${s.pct}%`);
  }
}
